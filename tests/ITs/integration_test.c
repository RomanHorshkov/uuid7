 ...
 *
 * The tests intentionally exercise the public API only. Imported-state
 * behavior is driven through uuid7_init(fn, last_gen_uuid7); there is no
 * separate public "resume" entry point.
 */
 ...
 * --------------------------
 *
 * Some tests need exact byte-by-byte control over the random tail. The helper
 * below feeds a scripted byte stream first, then increments from a fallback
...
/*
 * Thread worker context used by the concurrent uniqueness test.
 * Each worker writes into a disjoint slice of a shared output buffer.
 */
 ...
 * rng_prepare_script()
 * --------------------
 * Load scripted RNG state without installing the callback. This is useful for
 * tests that want uuid7_init() itself to install the RNG function.
 */
 ...
 * rng_load_script()
 * -----------------
 * Load the scripted byte stream and immediately install scripted_rng() as the
 * active RNG for the library.
 */
 ...
 * fast_rng()
 * ----------
 * Cheap deterministic RNG used by heavy uniqueness tests. It is intentionally
 * fast and predictable; these tests validate generator behavior, not entropy
 * quality.
 */
static int fast_rng(void* buf, const size_t n)
{
    ...
    for(size_t i = 0; i < n; ++i)
    {
        uint32_t v = atomic_fetch_add_explicit(&g_fast_rng_state, 1u,
 ...
 * zero_rng()
 * ----------
 * Return all-zero random bytes. This is ideal for tests that care only about
 * timestamp / sequence behavior and want the random tail to stay stable.
 */
 ...
 * --------------------------------------
 * Deterministic time sources used to force stable timestamps and simulated
 * clock regressions.
 */
...
{
    return (uint16_t)(((uint16_t)(uuid[6] & TEST_UUID_SEQ_HIGH_MASK) << 8) |
                      uuid[7]);
}
 ...
 * build_valid_uuid7()
 * -------------------
 * Build a minimal, syntactically valid UUIDv7 buffer with a caller-controlled
 * timestamp and sequence. The random tail is zeroed because imported-state
 * logic only consumes timestamp, version, sequence, and variant bits.
 */
static void build_valid_uuid7(uint8_t uuid[UUID7_SIZE_BYTES],
                              uint64_t ms,
                              uint16_t seq)
{
 ...
 * reset_state()
 * -------------
 * Reset both the library-under-test and all deterministic helper state so
 * every test starts from a clean, isolated baseline.
 */
 ...
 * ------------------------------------------
 * uuid7_gen() must remain usable even if the caller never explicitly calls
 * uuid7_init() or uuid7_set_rng_func().
 */
 ...
 * ------------------------------------
 * With a fixed millisecond, packed (ms, seq) state must be strictly
 * increasing for every generated UUID.
 */
 ...
 * -----------------------------------
 * A single logical millisecond can hold TEST_SEQ_SPACE UUIDs because the
 * sequence field is 12 bits wide (0..4095). The next UUID must roll into the
 * next logical millisecond and restart the sequence at zero.
 */
 ...
 * ---------------------------------
 * Passing NULL as the RNG in uuid7_init() must actively restore the default
 * RNG, not leave a previously installed custom RNG in place.
 */
 ...
 * ---------------------------------------
 * Importing a previous UUID through uuid7_init() must raise the internal
 * monotonic floor so the next generated UUID is strictly newer.
 */
 ...
 * -------------------------------------------------
 * Re-running uuid7_init() with an older imported UUID must never rewind the
 * generator's monotonic state.
 */
 ...
 * -----------------------------------------
 * The default-RNG failure hook must not affect callers that install a custom
 * RNG implementation.
 */
 ...
 * thread_generate()
 * -----------------
 * Worker body for the multithreaded integration test. Each thread asserts
 * local monotonicity and correct UUID markers while writing its slice.
 */
static void* thread_generate(void* arg)
{
    ...
    for(size_t i = 0; i < ctx->count; ++i)
    {
        ...
        if((uuid[6] & 0xF0u) != TEST_UUID_VERSION_TOP ||
           (uuid[8] & TEST_UUID_VARIANT_MASK) != TEST_UUID_VARIANT_TOP)
        {
            ctx->error = -4;
            return NULL;
        }
...
static void test_multithreaded_uniqueness(void** state)
{
    ...
    {
        assert_true(memcmp(all + ((i - 1u) * UUID7_SIZE_BYTES),
                           all + (i * UUID7_SIZE_BYTES),
                           UUID7_SIZE_BYTES) != 0);
    }
 ...
 * --------------------------------
 * Even if the wall clock moves backward, the packed monotonic state must keep
 * increasing.
 */
 ...
 * ------------------------------------
 * Repeated sequence exhaustion must keep advancing the logical millisecond one
 * step at a time with the sequence restarting from zero on each rollover.
 */
 ...
 * -------------------------------------
 * Stress the generator in one thread and confirm all produced UUIDs are
 * unique after lexicographic sorting.
 */
